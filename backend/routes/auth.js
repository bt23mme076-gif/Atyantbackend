...
//  (matches the "Send Verification Code" step in the frontend signup modal)
// ─────────────────────────────────────────────

// Step 1: validate details, stash a not-yet-verified user, email an OTP
router.post('/signup-initiate', async (req, res) => {
  try {
    ...
    // Someone already fully signed up with these details — reject as before.
    ...
    if (existingUser && !existingUser.isVerified) {
      // Unfinished signup attempt (e.g. they never entered the OTP last time) —
      // overwrite it with the latest details rather than erroring out.
    ...
    res.status(500).json({ message: 'Error initiating signup', error: error.message });
  }
});

// Optional: let the user request a fresh code if the first one expired
...
// Step 2: check the OTP, activate the account, and log them in
router.post('/signup-verify', async (req, res) => {
  try {
    ...
    const user = await User.findOne({ email, isVerified: false })
      .select('+signupOTP +signupOTPExpires');

    if (!user || user.signupOTP !== otp || user.signupOTPExpires < Date.now()) {
    ...
    // Attribute the signup to the referring mentor, same as the old direct /signup route.
    if (ref && typeof ref === 'string' && ref.toLowerCase() !== user.username.toLowerCase()) {
      User.updateOne(
        { username: new RegExp(`^${ref.trim()}$`, 'i') },
        { $inc: { referralSignups: 1 } }
      ).catch(err => console.error('referralSignups increment error:', err.message));
